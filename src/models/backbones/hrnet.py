...
class HighResolutionNet(nn.Module):
    ...
    def init_weights(
        ...
        if os.path.isfile(pretrained):
            ...
            # print(f"HRNet has {param_size(self)}M parameters")
...
class HRNet(HighResolutionNet):
    ...
    def __init__(self, cfg):
        ...
        hrnet_config = self.__get_hr_config(cfg.get("HRNET_TYPE", "w40"))
        super(HRNet, self).__init__(hrnet_config)
        self.init_weights(cfg.get("PRETRAINED", ""))
        self.name = type(self).__name__

    def __get_hr_config(self, hrnet_type):
        if hrnet_type == "w64":
            channels = [
                [64],
                [64, 128],
            ...
            raise Exception("HRNet only supports ['w64', 'w40'] as model_type, found: "+hrnet_type)
...
if __name__ == "__main__":
    ...
    cfg = {
        "PRETRAINED": True,  # give path to checkpoint here
        "HRNET_TYPE": "w40"
    }
