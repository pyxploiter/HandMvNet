# Source: https://github.com/JubSteven/POEM-v2/blob/release/lib/models/backbones/resnet.py
...
class ResNet(nn.Module):
    ...
    def __init__(self, version, block, layers, num_classes=1000, freeze_batchnorm=True, early_return=4, variant="default"):
    ...
    def load_pretrained(self, state_dict):
        print(f"Loading {self.name} pretrained models")
        if self.variant == "paper":
            print("[warning] ignoring backbone weights from layer 4 onwards")
            state_dict = {key: value for key, value in state_dict.items() if not key.startswith('layer4')}
...
class ResNet50_Paper(ResNet):

    def __init__(self, cfg):
        super(ResNet50_Paper, self).__init__(version="50",
                                       block=Bottleneck,
                                       layers=[3, 4, 6, 3],
                                       freeze_batchnorm=False,
                                       variant="paper")
        if cfg.get("PRETRAINED", False):
            self.load_pretrained(model_zoo.load_url(ResNet50_Weights.DEFAULT.url))
...
if __name__ == "__main__":
    ...
    for o in out:
        print(out[o].shape)
    # # output
    # odict_keys(['res_layer1', 'res_layer2', 'res_layer3', 'res_layer4', 'res_layer4_mean'])
